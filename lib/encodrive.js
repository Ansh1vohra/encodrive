...
export default class Encodrive {
  ...
  async uploadFile(file) {
    // 1. Request signed URL from your backend (USE application/json)
    ...
    // 4. Upload encrypted file directly to S3 (USE application/octet-stream)
    await axios.put(uploadURL, encryptedBuffer, {
      headers: { "Content-Type": "application/octet-stream" } // ← FIXED
    });
